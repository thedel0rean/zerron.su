...
long WINAPI C_Hooks::hkPresent( IDirect3DDevice9* pDevice, RECT* pSrcRect, RECT* pDescRect, HWND pHWND, RGNDATA* pRegion )
{
	...
	for (auto current : c_lua::get().hooks.getHooks(_S("Render")))
		current.func();
...
LRESULT WINAPI C_Hooks::hkWndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	g_Globals.m_KeyData.m_aHoldedKeys[18] = GetAsyncKeyState(VK_LMENU) || GetAsyncKeyState(VK_RMENU);
	...
	if (g_Menu->IsMenuOpened() && (msg == WM_LBUTTONDOWN || msg == WM_LBUTTONUP || msg == WM_MOUSEMOVE))
		return false;
