...
public:
	...
	static void __fastcall hkRunCommand(void* ecx, void* edx, C_BasePlayer* player, C_UserCmd* ucmd, C_MoveHelper* moveHelper);
	...
	static float __fastcall hkStudio_Duration(C_StudioHDR* pStudioHDR, int iSequence, const float poseParameter[]);
	static void	__fastcall	BuildTransform(void* this_pointer, void* edx, void* hdr, void* pos, void* q, const void* camera_transform, int bone_mask, void* bone_computed);
	static QAngle* __fastcall hkGetEyeAngles(void* ecx, void* edx);

	static void hkFlashDuration(const CRecvProxyData* Data, LPVOID pStruct, LPVOID pOut );
	static void m_flAbsYawHook(const CRecvProxyData* Data, LPVOID pStruct, LPVOID pOut);
	static void m_nViewModel(const CRecvProxyData* Data, LPVOID pStruct, LPVOID pOut);
	static void m_nSequence(const CRecvProxyData* Data, LPVOID pStruct, LPVOID pOut);
	...
	// model render
	static void __fastcall hkDrawModelExecute(LPVOID pEcx, uint32_t, LPVOID pResults, DrawModelInfo_t* pInfo, matrix3x4_t* pBoneToWorld, float_t* flpFlexWeights, float_t* flpFlexDelayedWeights, Vector& vrModelOrigin, int32_t iFlags);
	...
	static int __fastcall hkSendDatagram(void* netchan, void*, void* datagram);
	static void __fastcall hkProcessPacket(void* netchan, void* edx, void* packet, bool bHasHeader);
	...
	static unsigned long __fastcall GCRetrieveMessageHook(void* ecx, void*, uint32_t* punMsgType, void* pubDest, uint32_t cubDest, uint32_t* pcubMsgSize);
	static unsigned long __fastcall GCSendMessageHook(void* ecx, void*, uint32_t unMsgType, const void* pubData, uint32_t cubData);
