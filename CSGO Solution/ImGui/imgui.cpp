...
static ImVec4 BoxGaussianIntegral(ImVec4 x)
{
...
static ImVec4 BoxLinearInterpolation(ImVec4 x)
{
...
static float BoxShadow(ImVec2 lower, ImVec2 upper, ImVec2 point, float sigma, bool linearInterpolation)
{
...
void ImGui::DrawRectangleShadowVerticesAdaptive(RectangleShadowSettings& settings, ImVec2 pos, ImVec2 size)
{
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    child_window->DC.CursorPos += ImVec2(10, 35);
...
bool ImGui::BeginChildFrame(const char* label, ImGuiID id, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    ...
    bool ret = BeginChild(label, size, true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding | extra_flags);
    PopStyleVar(3);
    PopStyleColor();
    return ret;
}
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            {
                draw->AddRectFilled(ImVec2(wp.x, wp.y), ImVec2(wp.x + ws.x, wp.y + ws.y), ImColor(36, 36, 35, 255), 0.f);

                PushFont(ImGui::GetIO().Fonts->Fonts[1]);
                draw->AddRectFilled(ImVec2(wp.x + CalcTextSize(g_Menu->GetMenuName()).x + 20, wp.y), ImVec2(wp.x + +CalcTextSize(g_Menu->GetMenuName()).x + 21, wp.y + ws.y), ImColor(70, 70, 69), 0.f);
                
                ImVec2 iMainTextSize = ImGui::CalcTextSize(g_Menu->GetMenuName());
                draw->AddText(ImVec2(wp.x + 10, wp.y + 30 - iMainTextSize.y / 2), ImColor(200, 200, 200, alpha), g_Menu->GetMenuName());
                PopFont();


                //outline(window, 0, ImColor(70, 70, 69), 0.f);
                /*outline(window, 1, ImColor(96, 124, 255), 7.f);
                outline(window, 2, ImColor(70, 70, 69), 7.f);*/
            }
            else 
            {
                if (flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_Tooltip))
                {
                    draw->AddRectFilled(window->Pos, window->Pos + window->Size, ImColor(40, 40, 40, alpha), 5, ImDrawCornerFlags_All);
                    ...
                    draw->AddRect(wp, wp + ws, ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha), 5, ImDrawCornerFlags_All);
                }
                else if (flags & ImGuiWindowFlags_Popup)
                {
                    draw->AddRectFilled(window->Pos, window->Pos + window->Size, ImColor(36, 36, 36), 5, ImDrawCornerFlags_Bot);
                    draw->AddRect(wp, wp + ws, ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha), 5, ImDrawCornerFlags_All);
                }
            }
        }
...
void ImGui::DefaultRenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            window->DrawList->AddRectFilled(ImVec2(x, y), ImVec2(x + 181, y + 22), ImColor(36, 36, 36, 100));
            window->DrawList->AddRect(ImVec2(x, y), ImVec2(x + 181, y + 22), ImColor(60, 60, 60, 100));
            //window->DrawList->AddRectFilled(ImVec2(x, y + 22), ImVec2(x + 181, y + 23), ImColor(100, 100, 255, 255));
        }
...
void ImGui::End()
{
    ...
    if (window->Flags & ImGuiWindowFlags_ChildWindow)
    {
        if (!(window->Flags & ImGuiWindowFlags_NoName))
        {
            ...
            window->DrawList->AddRectFilled(window->Pos, window->Pos + ImVec2(window->Size.x, textSize.y + 6), ImColor(40, 40, 40, (int)(g.Style.Alpha * 255)), 5, ImDrawCornerFlags_All);
            window->DrawList->AddRectFilled(window->Pos + ImVec2(5, textSize.y + 6), window->Pos + ImVec2(window->Size.x - 5, textSize.y + 7), ImColor(60, 60, 60, (int)(g.Style.Alpha * 255)), 5, ImDrawCornerFlags_All);

            window->DrawList->AddText(window->Pos + ImVec2(10, 3), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), window->Name);

            window->DrawList->AddRect(window->Pos, window->Pos + window->Size, ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha), 5, ImDrawCornerFlags_All);
