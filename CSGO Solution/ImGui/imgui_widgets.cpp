...
bool ImGui::SkinCard(bool is_loading, const char* txt, int rarity, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col) {
    ...
    const ImVec4 hover_act = ImVec4(26 / 255.f, 26 / 255.f, 26 / 255.f, g.Style.Alpha);
    const ImVec4 hover_dis = ImVec4(36 / 255.f, 36 / 255.f, 36 / 255.f, g.Style.Alpha);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;

    //static std::map<ImGuiID, ImVec4> hover_color;custom_button_rect_hover_color
    auto it_hcolor = g_Globals.m_Animations.custom_button_hover_color.find(id);
    if (it_hcolor == g_Globals.m_Animations.custom_button_hover_color.end())
    {
        g_Globals.m_Animations.custom_button_hover_color.insert({ id, hover_dis });
        it_hcolor = g_Globals.m_Animations.custom_button_hover_color.find(id);
    }
    if (hovered || pressed)
    {
        ImVec4 to = hover_act;
    ...
    const ImVec4 hover_actr = ImVec4(96 / 255.f, 124 / 255.f, 255 / 255.f, g.Style.Alpha);
    const ImVec4 hover_disr = ImVec4(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha);

    auto it_hcolorr = g_Globals.m_Animations.custom_button_rect_hover_color.find(id);
    if (it_hcolorr == g_Globals.m_Animations.custom_button_rect_hover_color.end())
    {
        g_Globals.m_Animations.custom_button_rect_hover_color.insert({ id, hover_disr });
        it_hcolorr = g_Globals.m_Animations.custom_button_rect_hover_color.find(id);
    }
    if (hovered || pressed)
    {
        ImVec4 to = hover_actr;
        if (it_hcolorr->second.x != to.x)
        {
            if (it_hcolorr->second.x < to.x)
                it_hcolorr->second.x = ImMin(it_hcolorr->second.x + deltatime, to.x);
            else if (it_hcolorr->second.x > to.x)
                it_hcolorr->second.x = ImMax(to.x, it_hcolorr->second.x - deltatime);
        }

        if (it_hcolorr->second.y != to.y)
        {
            if (it_hcolorr->second.y < to.y)
                it_hcolorr->second.y = ImMin(it_hcolorr->second.y + deltatime, to.y);
            else if (it_hcolorr->second.y > to.y)
                it_hcolorr->second.y = ImMax(to.y, it_hcolorr->second.y - deltatime);
        }

        if (it_hcolorr->second.z != to.z)
        {
            if (it_hcolorr->second.z < to.z)
                it_hcolorr->second.z = ImMin(it_hcolorr->second.z + deltatime, to.z);
            else if (it_hcolorr->second.z > to.z)
                it_hcolorr->second.z = ImMax(to.z, it_hcolorr->second.z - deltatime);
        }

        if (it_hcolorr->second.w != to.w)
        {
            if (it_hcolorr->second.w < to.w)
                it_hcolorr->second.w = ImMin(it_hcolorr->second.w + deltatime, to.w);
            else if (it_hcolorr->second.w > to.w)
                it_hcolorr->second.w = ImMax(to.w, it_hcolorr->second.w - deltatime);
        }
    }
    else
    {
        ImVec4 to = hover_disr;
        if (it_hcolorr->second.x != to.x)
        {
            if (it_hcolorr->second.x < to.x)
                it_hcolorr->second.x = ImMin(it_hcolorr->second.x + deltatime, to.x);
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImGui::GetColorU32(it_hcolor->second), 5);
    window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(it_hcolorr->second), 5);
...
bool ImGui::CustomButton(const char* txt, const ImVec2& size)
{
    ...
    RenderText(ImVec2(bb.Min.x + size.x / 2 - tSize.x / 2, bb.Min.y + size.y / 2 - tSize.y / 2), s_txt.c_str()); //(ImVec2(bb.Min.x + size.x / 2 - tSize.x / 2, bb.Min.y + size.y / 2 - tSize.y / 2), GetColorU32(ImGuiCol_Text), s_txt.data());
...
void ImGui::TabButton(const char* label, const char* icon_name, int* selected, int num, int total) {
    ...
    ImVec2 pos_icon = window->Pos + ImVec2(10, 60) + ImVec2(0, (CalcTextSize(icon_name).y + 6) * num);
    ImVec2 size_icon = ImVec2(CalcTextSize(icon_name).x, CalcTextSize(icon_name).y + 6);
    PopFont();

    PushFont(ImGui::GetIO().Fonts->Fonts[1]);
    ImVec2 pos = window->Pos + ImVec2(10, 60) + ImVec2(0, (CalcTextSize(g_Menu->GetMenuName( )).y + 6) * num);
    ImVec2 size = ImVec2(CalcTextSize(g_Menu->GetMenuName( )).x + 10, CalcTextSize(g_Menu->GetMenuName( )).y + 6);
    PopFont();
    ...
    const ImVec4 hovered_circle_active = ImVec4(96 / 255.f, 124 / 255.f, 255 / 255.f, g.Style.Alpha);
    const ImVec4 hovered_circle_disable = ImVec4(96 / 255.f, 124 / 255.f, 255 / 255.f, 0.f);
    ...
    auto it_hcolor = g_Globals.m_Animations.tab_button_hover_color.find(id);
    if (it_hcolor == g_Globals.m_Animations.tab_button_hover_color.end())
    {
        g_Globals.m_Animations.tab_button_hover_color.insert({ id, hovered_circle_disable });
        it_hcolor = g_Globals.m_Animations.tab_button_hover_color.find(id);
    }
    if (*selected == num || hovered)
    {
        ...
        if (*selected == num)
            to = ImVec4(96 / 255.f, 124 / 255.f, 255 / 255.f, 1.f);
        else if (hovered)
            to = ImVec4(96 / 255.f, 124 / 255.f, 255 / 255.f, 0.3f);
    ...
    window->DrawList->AddRectFilled(ImVec2(pos_icon.x, pos.y + 3), ImVec2(pos_icon.x + size.x - 10, pos.y + size.y - 3), ImColor(40 / 255.f, 40 / 255.f, 40 / 255.f, it_hcolor->second.w), 7.f);
    window->DrawList->AddRect(ImVec2(pos_icon.x, pos.y + 3), ImVec2(pos_icon.x + size.x - 10, pos.y + size.y - 3), ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, it_hcolor->second.w), 7.f);
    ...
    window->DrawList->AddText(ImVec2(pos.x + 10 + size_icon.x + 5, pos.y + size.y / 2 - label_size.y / 2 - 4), ImColor(200, 200, 200, (int)(g.Style.Alpha * 255)), label);
    PopFont();
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto it_pos_active = g_Globals.m_Animations.checkbox_pos_active.find(id);
    auto it_size_active = g_Globals.m_Animations.checkbox_size_active.find(id);

    if (it_pos_active == g_Globals.m_Animations.checkbox_pos_active.end())
    {
        g_Globals.m_Animations.checkbox_pos_active.insert({ id, 3.f });
        it_pos_active = g_Globals.m_Animations.checkbox_pos_active.find(id);
    }
    if (it_size_active == g_Globals.m_Animations.checkbox_size_active.end())
    {
        g_Globals.m_Animations.checkbox_size_active.insert({ id, 6.f });
        it_size_active = g_Globals.m_Animations.checkbox_size_active.find(id);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    ...
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0, 3));
    ...
    const ImVec4 hover_dis = ImVec4(96 / 255.f, 124 / 255.f, 255 / 255.f, g.Style.Alpha);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;

    //static std::map<ImGuiID, ImVec4> hover_color;
    auto it_hcolor = g_Globals.m_Animations.begin_combo_hover_color.find(id);
    if (it_hcolor == g_Globals.m_Animations.begin_combo_hover_color.end())
    {
        g_Globals.m_Animations.begin_combo_hover_color.insert({ id, hover_act });
        it_hcolor = g_Globals.m_Animations.begin_combo_hover_color.find(id);
    }
    ...
    window->DrawList->AddRectFilled(cb_bb.Min, cb_bb.Max, ImColor(36, 36, 36, (int)(g.Style.Alpha * 255)), 5.f);

    window->DrawList->AddRect(cb_bb.Min, cb_bb.Max, ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha), 5.f);
    
    if (label_size.x > 0)
        RenderText(frame_bb.Min, label);

    window->DrawList->PushClipRect(cb_bb.Min, cb_bb.Max - ImVec2(20, 0), true);
    window->DrawList->AddText(cb_bb.Min + ImVec2(5, 1), ImColor(150 / 255.f, 150 / 255.f, 150 / 255.f, style.Alpha), preview_value);
    ...
    //static std::map<ImGuiID, float> size_active;
    auto it_size_active = g_Globals.m_Animations.begin_combo_size_active.find(id);

    if (it_size_active == g_Globals.m_Animations.begin_combo_size_active.end())
    {
        g_Globals.m_Animations.begin_combo_size_active.insert({ id, 0.f });
        it_size_active = g_Globals.m_Animations.begin_combo_size_active.find(id);
    }
...
void ImGui::MultiCombo(std::string name, bool variable[], const char* labels[], int count, std::string& preview)
{
    if (name.size() > 3)
    {
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    PopFont();
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 2), window->DC.CursorPos + ImVec2(label_size.x + 5, label_size.y + 4));
    const ImRect total_bb(window->DC.CursorPos + ImVec2(0, 2), window->DC.CursorPos + ImVec2(label_size.x + 5, label_size.y + 4));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &frame_bb))
        return false;
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min), ImVec2(frame_bb.Max), ImColor(36, 36, 36, (int)(g.Style.Alpha * 255)), 3);
    window->DrawList->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(60, 60, 60, (int)(g.Style.Alpha * 255)), 3);

    window->DrawList->AddText(frame_bb.Min + ImVec2(3, 0), g.ActiveId == id ? ImColor(96, 124, 255) : ImColor(90 / 255.f, 90 / 255.f, 90 / 255.f, g.Style.Alpha), buf_display);
    PopFont();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#')
        window->DrawList->AddText(frame_bb.Min, ImColor(150 / 255.f, 150 / 255.f, 150 / 255.f, g.Style.Alpha), label);
    ...
    auto it_size = g_Globals.m_Animations.slider_size.find(id);
    auto it_old_value = g_Globals.m_Animations.slider_old_value.find(id);
    auto it_active_value = g_Globals.m_Animations.slider_active_value.find(id);

    if (it_size == g_Globals.m_Animations.slider_size.end())
    {
        g_Globals.m_Animations.slider_size.insert({ id, 5.f });
        it_size = g_Globals.m_Animations.slider_size.find(id);
    }
    if (it_old_value == g_Globals.m_Animations.slider_old_value.end())
    {
        g_Globals.m_Animations.slider_old_value.insert({ id, 0 });
        it_old_value = g_Globals.m_Animations.slider_old_value.find(id);
    }
    if (it_active_value == g_Globals.m_Animations.slider_active_value.end())
    {
        g_Globals.m_Animations.slider_active_value.insert({ id, floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) });
        it_active_value = g_Globals.m_Animations.slider_active_value.find(id);
    }
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(36, 36, 36, (int)(g.Style.Alpha * 255)), 5);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(60, 60, 60, (int)(g.Style.Alpha * 255)), 5);

    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y), ImColor(96, 124, 255, (int)(g.Style.Alpha * 255)), 5);

    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y - 3), it_size->second, ImColor(96, 124, 255, (int)(g.Style.Alpha * 255)), 12);
    ...
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
    //static std::map<ImGuiID, ImVec4> hover_color;
    auto it_hcolor = g_Globals.m_Animations.slider_hover_color.find(id);
    if (it_hcolor == g_Globals.m_Animations.slider_hover_color.end())
    {
        g_Globals.m_Animations.slider_hover_color.insert({ id, hover_act });
        it_hcolor = g_Globals.m_Animations.slider_hover_color.find(id);
    }
    ...
    window->DrawList->AddText(ImVec2(slider_bb.Max.x + 10, slider_bb.Min.y - CalcTextSize(value_buf).y / 2), ImColor(150 / 255.f, 150 / 255.f, 150 / 255.f, g.Style.Alpha), value_buf);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, 240, (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y * 1.5f)); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(120, 0), window->DC.CursorPos + ImVec2(w, size.y));
    const ImRect total_bb(frame_bb.Min - ImVec2(120, 0), frame_bb.Max + ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, 0.0f));
    ...
    if (is_multiline)
    {
        ...
        if (!BeginChildFrame(label, id, frame_bb.GetSize()))
        {
            EndChildFrame();
            EndGroup();
            return false;
    ...
    if (!is_multiline)
    {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(it_hcolor->second), 5);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(it_hcolorr->second), 5);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, int expand)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    SameLine(window->Size.x - expand);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(20, 12)))
        {
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), rounding, 0);
        else
    ...
    window->DrawList->AddRect(bb_inner.Min, bb_inner.Max, ImColor(10, 12, 14, 255));
    window->DrawList->AddRect(bb_inner.Min - ImVec2(1, 1), bb_inner.Max + ImVec2(1, 1), ImColor(45, 47, 54));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec4 hover_dis = ImVec4(96 / 255.f, 124 / 255.f, 255 / 255.f, 1.f);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;

    //static std::map<ImGuiID, ImVec4> hover_color;
    auto it_hcolor = g_Globals.m_Animations.selectable_hover_color.find(id);
    if (it_hcolor == g_Globals.m_Animations.selectable_hover_color.end())
    {
        g_Globals.m_Animations.selectable_hover_color.insert({ id, hover_act });
        it_hcolor = g_Globals.m_Animations.selectable_hover_color.find(id);
    }
    if (hovered || selected)
    {
        ImVec4 to = hover_dis;
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    BeginGroup();
    BeginChildFrame(label, id, frame_bb.GetSize());

    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x + 10, frame_bb.Min.y + style.FramePadding.y), label);
