...
struct RectangleShadowSettings
{
    ...
    ImVec2  shadowSize = ImVec2(120, 120);
...
namespace ImGui
{
    ...
    IMGUI_API void			DrawRectangleShadowVerticesAdaptive(RectangleShadowSettings& settings, ImVec2 pos, ImVec2 size);
    ...
    IMGUI_API void          MultiCombo(std::string name, bool variable[], const char* labels[], int count, std::string& preview);
    ...
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.1f", float power = 1.0f);     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display. Use power!=1.0 for power curve sliders
    ...
    IMGUI_API bool          ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags = 0, int expand = 34);
    ...
    IMGUI_API bool          BeginChildFrame(const char* label, ImGuiID id, const ImVec2& size, ImGuiWindowFlags flags = 0); // helper to create a child window / scrolling region that looks like a normal widget frame
...
struct ImColor
{
    ...
    ImColor(const Color col) { float sc = 1.0f / 255.0f; Value.x = (float)col.r() * sc; Value.y = (float)col.g() * sc; Value.z = (float)col.b() * sc; Value.w = (float)col.a() * sc; }
...
struct ImDrawCmd
{
    ...
    ImDrawCmd() { ElemCount = 0; ClipRect.x = ClipRect.y = ClipRect.z = ClipRect.w = 0.0f; TextureId = (ImTextureID)NULL; VtxOffset = IdxOffset = 0;  UserCallback = NULL; UserCallbackData = NULL; blur_strength = 0;
    }
};
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddBlur(float blur);
