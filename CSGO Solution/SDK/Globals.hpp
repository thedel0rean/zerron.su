...
public:
	...
	std::array < std::array < float, 5 >, MAXSTUDIOPOSEPARAM > m_PoseParameters = { };
...
typedef float(__stdcall* Studio_Duration_t) (C_StudioHDR*, int, const float[]);
...
typedef void( __thiscall* ProcessPacket_t )(void*, void*, bool);
...
typedef void(__thiscall* DrawModelExecute_t)(LPVOID, LPVOID, DrawModelInfo_t*, matrix3x4_t*, float_t*, float_t*, Vector&, int32_t);
typedef void(__thiscall* Buildtransform_t)(void*, void*, void*, void*, const void*, int, void*);
...
public:
	...
	struct
	{
		...
		bool m_flVelocityModifierOverride = false;
	...
	{
		// did we break lby
		bool m_bDidBreakLBY = false;
		bool m_bAwaitingLBYUpdate = false;
		bool m_bShouldUpdateLowerBodyYaw = false;
		bool m_bDidShotAtChokeCycle = false;
		QAngle m_angLastAngles = QAngle(0, 0, 0);
		QAngle m_angChokedShotAngles = QAngle(0, 0, 0);
		// lby data
		float_t m_flNextLowerBodyYawUpdate = 0.0f;
		float_t m_flLastLBYDelta = 0.0f;
	...
	struct
	{
		...
		std::array < int, 65 > m_ResolverType = { };
		std::array < int, 65 > m_ResolverDelta = { };
		std::array < int, 65 > m_ResolverSide = { };
		std::array < std::array < int, 65 >, 65 > m_ResolverMissedShotSide = { };
	...
	struct
	{
		...
		uint8_t* m_StudioDuration = nullptr;
		...
		uint8_t* m_GetSequenceLinearMotion = nullptr;
		uint8_t* m_GetWeaponPrefix = nullptr;
	...
	struct
	{
		struct
		{
			...
			Studio_Duration_t m_Studio_Duration = NULL;
			...
			SendDatagram_t m_SendDatagram = NULL;
			ProcessPacket_t m_ProcessPacket = NULL;
			DrawModelExecute_t m_DrawModelExecute = NULL;
			...
			C_RecvHook* m_AbsAngles;
			C_RecvHook* m_ModelIndex;
			C_RecvHook* m_Sequence;
			WriteUsercmdDeltaToBuffer_t m_WriteUsercmdDeltaToBuffer = NULL;
			RunCommand_t m_RunCommand = NULL;
			...
			EyeAngles_t m_Eye_Angles = NULL;
			Buildtransform_t m_Build_Transform = NULL;
		} m_Originals;
	} m_Hooks;

	struct
	{
		float m_flAverageLatency[MAX_FLOWS] = {0.f, 0.f};
	...
	struct
	{
		C_BaseEntity* pBuildTransformationsEntity = nullptr;
		QAngle	vecBuildTransformationsAngles;
	} m_UpToDate; // note - AkatsukiSun: Mein Freund hat mir das gegeben.
