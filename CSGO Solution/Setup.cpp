...
void C_SetUp::Instance( HINSTANCE hInstance )
{
	...
	CreateDirectory(_S("selling.pw\\"), NULL);
	CreateDirectory(_S("selling.pw\\scripts\\"), NULL);
...
void C_SetUp::SetupHooks( )
{
	...
	LPVOID pRunCommand = (LPVOID)((*(uintptr_t**)(g_Globals.m_Interfaces.m_Prediction))[19]);
	...
	LPVOID pDrawModelExecute = (LPVOID)((*(uintptr_t**)(g_Globals.m_Interfaces.m_StudioRender))[29]);
	...
	//maybe crash here
	LPVOID pSendMessageTarget = (LPVOID)((*(uintptr_t**)(SteamGameCoordinator))[0]);
	LPVOID pRetriveMessageTarget = (LPVOID)((*(uintptr_t**)(SteamGameCoordinator))[2]);
	...
	LPVOID pSetupAliveLoop = g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll, _S("55 8B EC 51 56 8B 71 60 83 BE 9C 29 00 00 00 0F 84 9C"));
	...
	LPVOID pEyeAngles                   = g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "56 8B F1 85 F6 74 32" ) );
	LPVOID pBuildTransformation         = g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "55 8B EC 83 E4 F0 81 EC ? ? ? ? 56 57 8B F9 8B 0D ? ? ? ? 89 7C 24 28" ) );
	...
	CREATE_HOOK(pDrawModelExecute, C_Hooks::hkDrawModelExecute, g_Globals.m_Hooks.m_Originals.m_DrawModelExecute);
	...
	//CREATE_HOOK(pGetClientModelRenderable, C_Hooks::hkGetClientModelRenderable, g_Globals.m_Hooks.m_Originals.m_Useless); // note - AkatsukiSun: ok.
	...
	CREATE_HOOK(pRunCommand, C_Hooks::hkRunCommand, g_Globals.m_Hooks.m_Originals.m_RunCommand);
	CREATE_HOOK(pEyeAngles, C_Hooks::hkGetEyeAngles, g_Globals.m_Hooks.m_Originals.m_Eye_Angles);
	CREATE_HOOK(pBuildTransformation, C_Hooks::BuildTransform, g_Globals.m_Hooks.m_Originals.m_Build_Transform);
	//CREATE_HOOK(pStudio_Duration, C_Hooks::hkStudio_Duration, g_Globals.m_Hooks.m_Originals.m_Studio_Duration);
	//maybe crash here
	CREATE_HOOK(pSendMessageTarget, C_Hooks::GCSendMessageHook, g_Globals.m_Hooks.m_Originals.m_GCSendMessage);
	CREATE_HOOK(pRetriveMessageTarget, C_Hooks::GCRetrieveMessageHook, g_Globals.m_Hooks.m_Originals.m_GCRetrieveMessage);
	...
	g_Globals.m_Hooks.m_Originals.m_AbsAngles = new C_RecvHook(g_NetvarManager->GetNetProp(_S("DT_CSRagdoll"), _S("m_flAbsYaw")), C_Hooks::m_flAbsYawHook);
	g_Globals.m_Hooks.m_Originals.m_ModelIndex = new C_RecvHook(g_NetvarManager->GetNetProp(_S("DT_BaseViewModel"), _S("m_nModelIndex")), C_Hooks::m_nViewModel);
	g_Globals.m_Hooks.m_Originals.m_Sequence = new C_RecvHook(g_NetvarManager->GetNetProp(_S("DT_BaseViewModel"), _S("m_nSequence")), C_Hooks::m_nSequence);
...
void C_SetUp::SetupImGui()
{
	...
	ImGui::GetStyle( ).WindowRounding = 0.0f; //10f
...
void C_SetUp::SetupAddressList( )
{
	...
	g_Globals.m_AddressList.m_GetSequenceLinearMotion				=	g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll,  _S( "55 8B EC 83 EC 0C 56 8B F1 57 8B FA 85" ) );
	g_Globals.m_AddressList.m_GetWeaponPrefix						=	g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll,  _S( "53 56 57 8b f9 33 f6 8b 4f ? 8b 01 ff 90 ? ? ? ? 89 47" ) );
	g_Globals.m_AddressList.m_LookupSequence						=	g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll,  _S( "55 8b ec 56 8b f1 83 be ? ? ? ? ? 75 ? 8b 46 ? 8d 4e ? ff 50 ? 85 c0 74 ? 8b ce e8 ? ? ? ? 8b b6 ? ? ? ? 85 f6 74 ? 83 3e ? 74 ? 8b ce e8 ? ? ? ? 84 c0 74 ? ff 75" ) );
	g_Globals.m_AddressList.m_StudioDuration						=	g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll,  _S( "E8 ? ? ? ? 0F 57 C9 F3 0F 11 45 ? 83 C4 04" ) );
}
...
void C_SetUp::SetupConvarList( )
{
	...
	g_Globals.m_ConVars.m_clSideSpeed					= g_Globals.m_Interfaces.m_CVar->FindVar( _S( "cl_sidespeed" ) );
...
void C_SetUp::SetupInterfaceList( )
{
	...
	SteamUserHandle hSteamUser = ((SteamUserHandle(__cdecl*)(void))GetProcAddress(GetModuleHandle(_S("steam_api.dll")), _S("SteamAPI_GetHSteamUser")))();
	SteamPipeHandle hSteamPipe = ((SteamPipeHandle(__cdecl*)(void))GetProcAddress(GetModuleHandle(_S("steam_api.dll")), _S("SteamAPI_GetHSteamPipe")))();
	SteamClient = ((ISteamClient * (__cdecl*)(void))GetProcAddress(GetModuleHandle(_S("steam_api.dll")), _S("SteamClient")))();
	SteamGameCoordinator = (ISteamGameCoordinator*)SteamClient->GetISteamGenericInterface(hSteamUser, hSteamPipe, _S("SteamGameCoordinator001"));
	SteamUser = (ISteamUser*)SteamClient->GetISteamUser(hSteamUser, hSteamPipe, _S("SteamUser019"));
	SteamFriends = SteamClient->GetISteamFriends(hSteamUser, hSteamPipe, _S("SteamFriends015"));
	static auto SteamInventory = SteamClient->GetISteamInventory(hSteamUser, hSteamPipe, _S("STEAMINVENTORY_INTERFACE_V002"));
	auto _ = SteamFriends->GetLargeFriendAvatar(SteamUser->GetSteamID());
	SteamUtils = SteamClient->GetISteamUtils(hSteamPipe, _S("SteamUtils009"));
}