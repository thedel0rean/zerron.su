...
void C_FakeLag::Instance()
{
	...
	if ((*g_Globals.m_Interfaces.m_GameRules) && (*g_Globals.m_Interfaces.m_GameRules)->IsValveDS() && (g_Globals.m_Interfaces.m_ClientState->m_nChokedCommands() > 5 || g_Globals.m_Interfaces.m_ClientState->m_nChokedCommands() > 3 && g_Globals.m_LocalData.m_bAwaitingLBYUpdate))
		g_PacketManager->GetModifablePacket() = true;

	if (!(*g_Globals.m_Interfaces.m_GameRules) || !(*g_Globals.m_Interfaces.m_GameRules)->IsValveDS() && (g_Globals.m_Interfaces.m_ClientState->m_nChokedCommands() > 15 || g_Globals.m_Interfaces.m_ClientState->m_nChokedCommands() > 13 && g_Globals.m_LocalData.m_bAwaitingLBYUpdate))
		g_PacketManager->GetModifablePacket() = true;
