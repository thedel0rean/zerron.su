...
bool C_AutoWall::HandleBulletPenetration( )
{
	...
	int nEnterMaterial = g_Globals.m_Interfaces.m_PropPhysics->GetSurfaceData( m_PenetrationData.m_EnterTrace.surface.surfaceProps )->game.material;
	...
	float flDamageModifier = g_Globals.m_Interfaces.m_PropPhysics->GetSurfaceData( m_PenetrationData.m_EnterTrace.surface.surfaceProps )->game.flDamageModifier;
	float flPenetrationModifier = g_Globals.m_Interfaces.m_PropPhysics->GetSurfaceData( m_PenetrationData.m_EnterTrace.surface.surfaceProps )->game.flPenetrationModifier;

	int iEnterMaterial = g_Globals.m_Interfaces.m_PropPhysics->GetSurfaceData( m_PenetrationData.m_EnterTrace.surface.surfaceProps )->game.material;
	...
	else if ( iEnterMaterial == CHAR_TEX_FLESH && g_Globals.m_ConVars.m_DamageReduction->GetFloat( ) == 0 
	...
	{
		if ( g_Globals.m_ConVars.m_DamageReduction->GetFloat( ) == 0 )
			return true;

		flPenetrationModifier = g_Globals.m_ConVars.m_DamageReduction->GetFloat( );
		flDamageModifier = g_Globals.m_ConVars.m_DamageReduction->GetFloat( );
	}
	else
	{
		float flExitPenetrationModifier = g_Globals.m_Interfaces.m_PropPhysics->GetSurfaceData( m_PenetrationData.m_ExitTrace.surface.surfaceProps )->game.flPenetrationModifier;
		float flExitDamageModifier = g_Globals.m_Interfaces.m_PropPhysics->GetSurfaceData( m_PenetrationData.m_ExitTrace.surface.surfaceProps )->game.flDamageModifier;
	...
	int iExitMaterial = g_Globals.m_Interfaces.m_PropPhysics->GetSurfaceData( m_PenetrationData.m_ExitTrace.surface.surfaceProps )->game.material;
...
bool C_AutoWall::TraceToExit( Vector vecStart, Vector& vecEnd, CGameTrace* pEnterTrace, CGameTrace* pExitTrace )
{
	...
	while ( flDistance <= 90.0f )
	{
		...
		// trace line
		g_Globals.m_Interfaces.m_EngineTrace->TraceLine( vecEnd, vecTraceEnd, 0x4600400B, g_Globals.m_LocalPlayer, NULL, pExitTrace );
		...
		if ( pExitTrace->startsolid && pExitTrace->surface.flags & SURF_HITBOX )
		{
			...
			g_Globals.m_Interfaces.m_EngineTrace->TraceRay( Ray, 0x600400B, ( CTraceFilter* )( aSkipTwoEntities.data( ) ), pExitTrace );
...
bool C_AutoWall::SimulateFireBullet( )
{
	...
	while ( m_PenetrationData.m_PenetrationCount > 0 )
	{
		...
		g_Globals.m_Interfaces.m_EngineTrace->TraceRay( Ray, MASK_SHOT_HULL | CONTENTS_HITBOX, ( CTraceFilter* )( aSkipTwoEntities.data( ) ), &m_PenetrationData.m_EnterTrace );
...
bool C_AutoWall::IsPenetrablePoint( Vector vecShootPosition, Vector vecTargetPosition )
{
	...
	CGameTrace Trace;
	g_Globals.m_Interfaces.m_EngineTrace->TraceLine( vecShootPosition, vecTargetPosition, MASK_SHOT_HULL | CONTENTS_HITBOX, g_Globals.m_LocalPlayer, NULL, &Trace );
