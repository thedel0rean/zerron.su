...
namespace NewAutoWall
{
	...
	static void ClipTraceToPlayers(const Vector& vecAbsStart, const Vector& vecAbsEnd, const unsigned int fMask, CTraceFilter* pFilter, CGameTrace* pTrace, const float flMinRange = 0.0f)
	{
		...
		for (int iPlayerID = 1; iPlayerID <= g_Globals.m_Interfaces.m_GlobalVars->m_iMaxClients; iPlayerID++)
		{
			C_BasePlayer* pEntity = C_BasePlayer::GetPlayerByIndex(iPlayerID);
			...
			g_Globals.m_Interfaces.m_EngineTrace->ClipRayToEntity(ray, fMask | CONTENTS_HITBOX, pEntity, &trace);
	...
	static bool TraceToExit(CGameTrace& enterTrace, CGameTrace& exitTrace, Vector& vecStart, Vector& vecPosition, Vector& vecDirection, C_BasePlayer* pClipPlayer)
	{
		...
		while (flDistance <= 90.0f)
		{
			...
			vecStart = vecPosition + vecDirection * flDistance;

			if (!iStartContents)
				iStartContents = g_Globals.m_Interfaces.m_EngineTrace->GetPointContents(vecStart, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr);

			const int iCurrentContents = g_Globals.m_Interfaces.m_EngineTrace->GetPointContents(vecStart, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr);
			...
			if (!(iCurrentContents & MASK_SHOT_HULL) || (iCurrentContents & CONTENTS_HITBOX && iCurrentContents != iStartContents))
			{
				...
				g_Globals.m_Interfaces.m_EngineTrace->TraceRay(rayWorld, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr, &exitTrace);

				if (static auto sv_clip_penetration_traces_to_players = g_Globals.m_Interfaces.m_CVar->FindVar(_S("sv_clip_penetration_traces_to_players")); sv_clip_penetration_traces_to_players != nullptr && sv_clip_penetration_traces_to_players->GetBool())
				{
					CTraceFilter filter;
				...
				if (exitTrace.startsolid && exitTrace.surface.flags & SURF_HITBOX)
				{
					...
					g_Globals.m_Interfaces.m_EngineTrace->TraceRay(ray, MASK_SHOT_HULL, &filter, &exitTrace);
				...
				if (exitTrace.DidHit() && !exitTrace.startsolid)
				{
					...
					if (((C_BaseEntity*)(enterTrace.hit_entity))->IsBreakableEntity() && ((C_BaseEntity*)exitTrace.hit_entity)->IsBreakableEntity())
						return true;
				...
				if (!exitTrace.DidHit() || exitTrace.startsolid)
				{
					if (enterTrace.hit_entity != nullptr && ((C_BaseEntity*)enterTrace.hit_entity)->EntIndex() != 0 && ((C_BaseEntity*)(enterTrace.hit_entity))->IsBreakableEntity())
					{
	...
	static void run_penetration(pen_argument_t* argument) {
		...
		while (damage > 0.f && (damage * 4) >= argument->m_damage_penetration && (damage * 4) >= argument->m_damage) {
			...
			if (!TraceToExit(trace, exit_trace, pen_end, trace.endpos, dir, argument->m_from)) {
