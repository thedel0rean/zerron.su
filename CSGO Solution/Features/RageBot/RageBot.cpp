...
void C_RageBot::UpdatePeekState()
{
	...
	for (int32_t PlayerID = 1; PlayerID <= g_Globals.m_Interfaces.m_GlobalVars->m_iMaxClients; PlayerID++)
	{
		...
		m_RageSettings.m_Hitboxes[2] = true;
		m_RageSettings.m_Hitboxes[3] = true;
		m_RageSettings.m_Hitboxes[4] = true;
		m_RageSettings.m_Hitboxes[5] = true;
		...
		m_RageSettings.m_Multipoints[5] = true;
...
bool IsBody(int hitbox) 
{
	switch (hitbox)
	{
	case HITBOX_STOMACH: return true; break;
	case HITBOX_PELVIS: return true; break;
	//case HITBOX_LOWER_CHEST: return true; break;
	//case HITBOX_CHEST: return true; break;
	default: return false; break;
	}
}

float GetLethalDamage(C_BasePlayer* pPlayer, int weapon_index)
{
	switch (weapon_index)
	{
	case WEAPON_SCAR20:
	...
	case WEAPON_AWP:
		...
		break;
	default:
		return g_RageBot->GetMinDamage(pPlayer, false) / 1.5f;
		break;
	}
}
...
C_HitboxData C_RageBot::ScanPlayerRecord(C_BasePlayer* pPlayer, C_LagRecord LagRecord, Vector vecStartPosition)
{
	...
	if (m_RageSettings.m_Hitboxes[2])
	{
		...
		aHitboxesData.emplace_back(C_HitboxHitscanData(HITBOX_RIGHT_UPPER_ARM, m_RageSettings.m_SafeHitboxes[2] || bForcedSafety));
		aHitboxesData.emplace_back(C_HitboxHitscanData(HITBOX_LEFT_UPPER_ARM, m_RageSettings.m_SafeHitboxes[2] || bForcedSafety));
		aHitboxesData.emplace_back(C_HitboxHitscanData(HITBOX_RIGHT_HAND, m_RageSettings.m_SafeHitboxes[2] || bForcedSafety));
		aHitboxesData.emplace_back(C_HitboxHitscanData(HITBOX_LEFT_HAND, m_RageSettings.m_SafeHitboxes[2] || bForcedSafety));
	...
	if (m_RageSettings.m_Hitboxes[5])
	{
		...
		aHitboxesData.emplace_back(C_HitboxHitscanData(HITBOX_RIGHT_FOOT, m_RageSettings.m_SafeHitboxes[5] || bForcedSafety));
		aHitboxesData.emplace_back(C_HitboxHitscanData(HITBOX_LEFT_FOOT, m_RageSettings.m_SafeHitboxes[5] || bForcedSafety));
	...
	for (auto Hitbox : aHitboxesData)
	{
		...
		for (auto Point : m_Hitboxes)
		{
			...
			//penetration::PenetrationOutput_t out;
				...
				/*if (Hitbox.m_iHitbox == HITBOX_HEAD && out.m_hitgroup != HITGROUP_HEAD)
					continue;*/

				if (m_RageSettings.m_bLethalSafety && flDamage > pPlayer->m_iHealth() && pPlayer->m_iHealth() <= GetLethalDamage(pPlayer, pCombatWeapon->m_iItemDefinitionIndex()))
				{
					if (!ScanData.m_bIsSafeHitbox)
						continue;
				}
				...
				if (!ScanData.m_bIsSafeHitbox && g_Globals.m_ResolverData.m_MissedShots[pPlayer->EntIndex()] > 2)
					continue;
...
int32_t C_RageBot::GetMinDamage(C_BasePlayer* pPlayer, bool pen)
{
	int32_t iMinDamage = pen ? m_RageSettings.m_iMinPenDamage : m_RageSettings.m_iMinDamage;
...
void C_RageBot::OnNetworkUpdate(ClientFrameStage_t Stage)
{
	...
	for (int i = 0; i < g_Globals.m_ShotData.size(); i++)
	{
		...
		if (Shot->m_bDidIntersectPlayer)
		{
			...
			g_Globals.m_ResolverData.m_ResolverMissedShotSide.at(Shot->m_Target.m_LagRecord.m_RotationMode).at(Shot->m_Target.m_Player->EntIndex())++;
			std::stringstream asd;

			asd << _S("Missed shot due to animation desync | delta: ") << g_Globals.m_ResolverData.m_ResolverDelta[Shot->m_Target.m_Player->EntIndex()] << _S(" | resolver method: ") << g_Globals.m_ResolverData.m_ResolverType[Shot->m_Target.m_Player->EntIndex()] << _S(" | resolver side: ") << g_Globals.m_ResolverData.m_ResolverSide[Shot->m_Target.m_Player->EntIndex()] << _S(" | anim resolved: ") << g_Globals.m_ResolverData.m_AnimResoled[Shot->m_Target.m_Player->EntIndex()];

			if (g_Settings->m_aLogs[4])
				g_LogManager->PushLog(asd.str().c_str(), _S("b"));
		}
		else if (bMissedShotDueToOcclusion)
		{
			if (g_Settings->m_aLogs[4])
				g_LogManager->PushLog(_S("Missed shot due to occlusion error"), _S("b"));
		}
		else
		{
			if (g_Settings->m_aLogs[4])
				g_LogManager->PushLog(_S("Missed shot due to weapon spread"), _S("b"));
		}
...
std::vector < Vector > C_RageBot::GetHitboxPoints(C_BasePlayer* pPlayer, C_LagRecord LagRecord, Vector vecStartPosition, int32_t iHitbox)
{
	...
	if (iHitbox == HITBOX_HEAD)
	{
		...
		aMultipoints.emplace_back(vecCenter);
		aMultipoints.emplace_back(vecCenter + vecTop * (flScale / 100.0f));
		aMultipoints.emplace_back(vecCenter + vecTop + vecRight * (flScale / 120.0f));
		aMultipoints.emplace_back(vecCenter + vecTop + vecLeft * (flScale / 120.0f));
	...
	else if (iHitbox == HITBOX_CHEST)
	{
		...
		float_t flModifier = (m_RageSettings.m_iBodyScale / 100.0f);
		if (iAngleToPlayer < 140 && iAngleToPlayer > 30)
			flModifier = (m_RageSettings.m_iBodyScale / 100.0f);

		aMultipoints.emplace_back(vecCenter + Vector(0, 0, 0));
		aMultipoints.emplace_back(vecCenter + vecRight * flModifier);
		aMultipoints.emplace_back(vecCenter + vecLeft * flModifier);
	}
	else if (iHitbox == HITBOX_STOMACH)
	{
		float_t flModifier = m_RageSettings.m_iBodyScale / 100.0f;
		if (iAngleToPlayer < 120 && iAngleToPlayer > 60)
			flModifier = (m_RageSettings.m_iBodyScale / 100.0f) - 0.10f;
	...
	else if (iHitbox == HITBOX_PELVIS)
	{
		...
		aMultipoints.emplace_back(vecCenter - Vector(0.0f, 0.0f, 0.f));
		aMultipoints.emplace_back(vecCenter + vecRight * flModifier);
		aMultipoints.emplace_back(vecCenter + vecLeft * flModifier);
	}
	...
	else if (iHitbox == HITBOX_LEFT_FOREARM || iHitbox == HITBOX_RIGHT_FOREARM || iHitbox == HITBOX_RIGHT_HAND || iHitbox == HITBOX_LEFT_HAND || iHitbox == HITBOX_RIGHT_UPPER_ARM || iHitbox == HITBOX_LEFT_UPPER_ARM)
		aMultipoints.emplace_back(vecCenter - (iHitbox == HITBOX_LEFT_FOREARM || HITBOX_LEFT_HAND || HITBOX_LEFT_UPPER_ARM ? vecLeft : -vecLeft));
