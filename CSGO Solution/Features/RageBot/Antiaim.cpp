...
void C_AntiAim::Instance( )
{
	...
	g_Globals.m_LocalData.m_bAwaitingLBYUpdate = TICKS_TO_TIME(g_Globals.m_LocalPlayer->m_nTickBase()) + 1 >= g_Globals.m_LocalData.m_flNextLowerBodyYawUpdate;
	...
	if ( m_AntiAimConditions[ nCommand % MULTIPLAYER_BACKUP ] )
	{
		if (m_AntiAimConditions[nCommand % MULTIPLAYER_BACKUP] == ANTIAIM_FULL)
		{
			if (g_Settings->m_bAntiAimAtTargets && m_iManualSide > 3)
				flFinalYaw = this->GetTargetYaw();
			else if (m_AntiAimConditions[nCommand % MULTIPLAYER_BACKUP] == ANTIAIM_FULL)
				flFinalYaw += 180.0f;


			static bool right, left, back = false;
			if (GetAsyncKeyState(g_Settings->m_aManualLeft.Get().m_iKeySelected))
			{
				if (m_iManualSide == -1)
					m_iManualSide = INT_MAX;
				else
					m_iManualSide = -1;
			}
			else if (GetAsyncKeyState(g_Settings->m_aManualRight.Get().m_iKeySelected))
			{
				if (m_iManualSide == 1)
					m_iManualSide = INT_MAX;
				else
			...
			else if (GetAsyncKeyState(g_Settings->m_aManualBack.Get().m_iKeySelected))
			{
				if (m_iManualSide == 0)
					m_iManualSide = INT_MAX;
				else
			...
			if (m_iManualSide < INT_MAX)
				flFinalYaw += 90.0f * m_iManualSide;
		}
	...
	if (g_Settings->m_iLbyMode && g_ExploitSystem->m_Data.m_Selected == 0)
	{
		if (g_Globals.m_LocalPlayer->m_fFlags() & FL_ONGROUND)
		{
			if (this->CanBreakLowerBodyYaw())
			{
				g_Globals.m_LocalData.m_bDidBreakLBY = true;
				switch (g_Settings->m_iLbyMode)
				{
				case 1: g_PacketManager->GetModifableCommand()->m_angViewAngles.yaw += 120.0f * m_iDesyncSide; break;
				case 2: g_PacketManager->GetModifableCommand()->m_angViewAngles.yaw += g_PacketManager->GetModifableCommand()->m_nCommand & 1 ? 120.0f : -120.0f; break;
				default: break;
				}
...
bool C_AntiAim::CanBreakLowerBodyYaw()
{
	if (g_Globals.m_LocalPlayer->m_fFlags() & FL_ONGROUND)
		if (g_Globals.m_LocalPlayer->m_vecVelocity().Length() < 0.1f)
			if (g_Globals.m_LocalData.m_flLastLBYDelta > 35.0f)
				if (g_Globals.m_Interfaces.m_ClientState->m_nChokedCommands() < 13)
					return g_Globals.m_Interfaces.m_GlobalVars->m_flCurTime >= g_Globals.m_LocalData.m_flNextLowerBodyYawUpdate;
...
float C_AntiAim::GetTargetYaw( )
{
	...
	for ( int32_t i = 1; i < 65; i++ )
	{
		...
		QAngle angles;
		g_Globals.m_Interfaces.m_EngineClient->GetViewAngles(&angles);

		Vector calculated_angles = Vector(Math::CalcAngle(g_Globals.m_LocalPlayer->GetShootPosition(), pPlayer->GetAbsOrigin()).pitch, Math::CalcAngle(g_Globals.m_LocalPlayer->GetShootPosition(), pPlayer->GetAbsOrigin()).yaw, Math::CalcAngle(g_Globals.m_LocalPlayer->GetShootPosition(), pPlayer->GetAbsOrigin()).roll);

		auto fov = Math::GetFov(Vector(angles.pitch, angles.yaw, angles.roll), calculated_angles);

		if (fov < flBestDistance)
		{
			flBestDistance = fov;
...
{
	//if ( !g_Settings->m_bAntiAim || g_PacketManager->GetModifableCommand( )->m_nButtons & ( IN_MOVELEFT | IN_MOVERIGHT | IN_BACK | IN_FORWARD ) && m_Settings.m_iLBYMode)
	//	return;
	if (!g_Settings->m_bAntiAim || ((g_Globals.m_LocalPlayer->m_fFlags() & FL_ONGROUND) && g_Settings->m_iLbyMode && !g_ExploitSystem->m_Data.m_Selected))
		return;

	if (g_PacketManager->GetModifableCommand()->m_nButtons & (IN_MOVELEFT | IN_MOVERIGHT | IN_BACK | IN_FORWARD))
		return;
