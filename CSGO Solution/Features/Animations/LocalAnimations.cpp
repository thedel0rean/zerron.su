...
void C_LocalAnimations::Instance( )
{
	...
	if (!g_Globals.m_LocalData.m_bDidBreakLBY || !g_AntiAim->m_AntiAimConditions[g_PacketManager->GetModifableCommand()->m_nCommand % MULTIPLAYER_BACKUP])
		g_Globals.m_LocalData.m_angLastAngles = g_PacketManager->GetModifableCommand()->m_angViewAngles;
	else
		g_Globals.m_LocalData.m_flLowerBodyYaw = g_Globals.m_LocalPlayer->m_flLowerBodyYaw();
	...
	else if ( flCurtime > g_Globals.m_LocalData.m_flNextLowerBodyYawUpdateTime )
	{
		...
		g_Globals.m_LocalData.m_flLastLBYDelta = flAngleDifference;
	...
	g_Globals.m_LocalData.m_bShouldUpdateLowerBodyYaw = false;
}
