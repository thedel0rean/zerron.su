...
void c_kit_parser::setup() noexcept {
	...
	const auto item_schema = reinterpret_cast<CCStrike15ItemSchema*>(std::uintptr_t(item_system_fn()) + sizeof(void*));
	{
		...
		const auto get_paint_kit_definition_fn = reinterpret_cast<PaintKit_t * (__thiscall*)(CCStrike15ItemSchema*, int)>(sig_address + 11 + 5 + get_paint_kit_definition_offset);
		...
		const auto map_head = reinterpret_cast<head_t<int, PaintKit_t*>*>(std::uintptr_t(item_schema) + head_offset);
