...
void C_LogManager::Instance( )
{
	for ( int32_t i = 0; i < m_Logs.size( ); i++ )
	{
		...
		Color HurtColor = Color(62, 227, 30, (int)(LogData->m_flTextAlpha)); //175, 245, 83
		Color HarmColor = Color(245, 83, 83, (int)(LogData->m_flTextAlpha));
		if ( !LogData->m_bPrinted )
		{
			g_Globals.m_Interfaces.m_CVar->ConsoleColorPrintf( aColor, _S( "[ " ) );
			g_Globals.m_Interfaces.m_CVar->ConsoleColorPrintf( aColor, _S( "CSGO" ) );
			g_Globals.m_Interfaces.m_CVar->ConsoleColorPrintf( aColor, _S( " ] " ) );
		...
		if (LogData->m_LogIcon == _S("h"))
			g_Render->RenderText(LogData->m_LogIcon, ImVec2(4, flSpacing + 2), HurtColor, false, false, g_Globals.m_Fonts.m_LogIcons);

		if (LogData->m_LogIcon == _S("j"))
			g_Render->RenderText(LogData->m_LogIcon, ImVec2(4, flSpacing + 2), HarmColor, false, false, g_Globals.m_Fonts.m_LogIcons);
