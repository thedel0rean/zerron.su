...
void C_Chams::OnModelRender(LPVOID pResults, DrawModelInfo_t* pInfo, matrix3x4_t* aMatrix, float* flpFlexWeights, float* flpFlexDelayedWeights, Vector& vrModelOrigin, int iFlags)
{
	bool bIsPlayer = strstr(pInfo->m_pStudioHdr->szName, _S("models/player"));
	bool bIsArm = strstr(pInfo->m_pStudioHdr->szName, _S("arms"));
	bool bIsWeapon = strstr(pInfo->m_pStudioHdr->szName, _S("weapons/v_"));

	C_BaseEntity* pBaseEntity = reinterpret_cast <C_BaseEntity*> (pInfo->m_pClientEntity->GetIClientUnknown()->GetBaseEntity());
	if (pBaseEntity)
	{
	...
	if (bIsPlayer)
	{
		...
		if (!pPlayer && !(pPlayer->GetClientClass()->m_ClassID == ClassId_CCSRagdoll && g_Settings->m_bDrawRagdolls))
		{
			g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);
			return g_Globals.m_Interfaces.m_StudioRender->ForcedMaterialOverride(nullptr);
		}
		...
		if (aInvisible.m_bRenderChams)
			this->DrawModel(pPlayer, aInvisible, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags, true, true);

		if (g_Settings->m_aChamsSettings[7].m_bRenderChams)
			if (g_Globals.m_LocalPlayer == pPlayer)
				this->DrawModel(pPlayer, g_Settings->m_aChamsSettings[7], pResults, pInfo, g_LocalAnimations->GetDesyncMatrix().data(), flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags, true, false);
		...
		if (g_Settings->m_aChamsSettings[2].m_bRenderChams)
		{
			...
			if (this->GetInterpolatedMatrix(pPlayer, aBacktrackMatrix.data()))
				this->DrawModel(pPlayer, g_Settings->m_aChamsSettings[2], pResults, pInfo, aBacktrackMatrix.data(), flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags, true, true);
		}

		if (aVisible.m_bRenderChams)
			this->DrawModel(pPlayer, aVisible, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags, false, false);
	}
	else if (bIsArm && g_Settings->m_aChamsSettings[8].m_bRenderChams)
		this->DrawModel(NULL, g_Settings->m_aChamsSettings[8], pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags, true, false);

	g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);
	return g_Globals.m_Interfaces.m_StudioRender->ForcedMaterialOverride(nullptr);
}

void C_Chams::DrawModel(C_BaseEntity* pEntity, C_ChamsSettings Settings, LPVOID pResults, DrawModelInfo_t* pInfo, matrix3x4_t* aMatrix, float* flpFlexWeights, float* flpFlexDelayedWeights, Vector& vrModelOrigin, int iFlags, bool bForceNull, bool bXQZ)
{
	this->ApplyMaterial(Settings.m_iMainMaterial, bXQZ, Color(Settings.m_Color));
	g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);

	if (Settings.m_aModifiers[0])
	{
		this->ApplyMaterial(2, bXQZ, Color(Settings.m_aModifiersColors[0]));
		g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);
	}

	if (Settings.m_aModifiers[1])
	{
		this->ApplyMaterial(3, bXQZ, Color(Settings.m_aModifiersColors[1]));
		g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);
	}

	if (Settings.m_aModifiers[2])
	{
		this->ApplyMaterial(4, bXQZ, Color(Settings.m_aModifiersColors[2]));
		g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);
	}

	if (Settings.m_aModifiers[3])
	{
		this->ApplyMaterial(5, bXQZ, Color(Settings.m_aModifiersColors[3]));
		g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);
	}

	if (Settings.m_aModifiers[4])
	{
		this->ApplyMaterial(6, bXQZ, Color(Settings.m_aModifiersColors[4]));
		g_Globals.m_Hooks.m_Originals.m_DrawModelExecute(g_Globals.m_Interfaces.m_StudioRender, pResults, pInfo, aMatrix, flpFlexWeights, flpFlexDelayedWeights, vrModelOrigin, iFlags);
	}
	...
	g_Globals.m_Interfaces.m_StudioRender->SetAlphaModulation(1.0f);
	g_Globals.m_Interfaces.m_StudioRender->SetColorModulation(aWhite);

	return g_Globals.m_Interfaces.m_StudioRender->ForcedMaterialOverride(NULL);
}
...
#define CREATE_MATERIAL( var, name, content ) C_KeyValues* MACRO_CONCAT( KV, var ) = new C_KeyValues( name ); \
	...
	while( !MACRO_CONCAT( KV, var ) ) Sleep( 100 ); \
    var = g_Globals.m_Interfaces.m_MaterialSystem->CreateMaterial( name, MACRO_CONCAT( KV, var ) ); \
	while ( !var ) { Sleep( 100 ); var = g_Globals.m_Interfaces.m_MaterialSystem->CreateMaterial( name, MACRO_CONCAT( KV, var ) ); }\
...
void C_Chams::CreateMaterials()
{
	...
	CREATE_MATERIAL(m_pSkyMaterial, _S("ogclub_space"), _S(R"#( "UnlitTwoTexture" 
        {
 "$additive" "1"
          "$envmap" "models/effects/cube_white"
          "$envmaptint" "[1 1 1]"
...
void C_Chams::ApplyMaterial(int32_t iMaterial, bool bIgnoreZ, Color aColor, bool bCustom, bool bShotChams)
{
	...
	case 2: pRenderMaterial = m_pSkyMaterial; break;
	case 3: pRenderMaterial = m_pGlowMaterial; break;
	case 4: pRenderMaterial = m_pGhostMaterial; break;
	...
	g_Globals.m_Interfaces.m_StudioRender->SetColorModulation(aBlendColor);
	g_Globals.m_Interfaces.m_StudioRender->SetAlphaModulation(aColor.a() / 255.0f);
	return g_Globals.m_Interfaces.m_StudioRender->ForcedMaterialOverride(pRenderMaterial);
}
