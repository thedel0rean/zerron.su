...
void C_PlayerESP::RenderOutOfView( C_BasePlayer* pPlayer )
{
	...
	g_Render->RenderArc( iScreenSizeX / 2, iScreenSizeY / 2, 256, angle - width, angle + width, g_Settings->m_aOutOfViewArrows, 4.f) ;
	g_Render->RenderArc( iScreenSizeX / 2, iScreenSizeY / 2, 250, angle - width, angle + width, Color(g_Settings->m_aOutOfViewArrows.r(), g_Settings->m_aOutOfViewArrows.g(), g_Settings->m_aOutOfViewArrows.b(), 125), 1.5f);
}
...
void C_PlayerESP::RenderFlags(C_BasePlayer* pPlayer)
{
	...
	{
		g_Render->RenderText( pPlayer->m_bHasHelmet( ) && pPlayer->m_ArmourValue( ) > 0 ? _S( "HK" ) : ( pPlayer->m_bHasHelmet( ) ? _S( "H" ) : _S( "K" ) ), ImVec2( aBox.right + 3, aBox.top + iOffset ), Color(82, 155, 255), false, true, g_Globals.m_Fonts.m_SegoeUI );
		iOffset += 12;
	}

	if ( pPlayer->m_flFlashTime() > 0.1f && m_aPlayerData[pPlayer->EntIndex()].m_aSettings.m_Flags[2])
	{
		g_Render->RenderText( _S( "FLASHED" ), ImVec2( aBox.right + 3, aBox.top + iOffset ), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI );
		iOffset += 12;
	}
	/*for (int i = 5; i < 13; i++)
	{
		auto layer = pPlayer->m_AnimationLayers()[i];

		std::stringstream money1;
		money1 << _S("layer: ") << i << "  " << _S("m_flPlaybackRate:") << layer.m_flPlaybackRate;

		std::stringstream money2;
		money2 << _S("layer: ") << i << "  " << _S("m_flWeight:") << layer.m_flWeight;

		std::stringstream money3;
		money3 << _S("layer: ") << i << "  " << _S("m_flWeightDeltaRate:") << layer.m_flWeightDeltaRate;

		std::stringstream money4;
		money4 << _S("layer: ") << i << "  " << _S("m_nSequence:") << layer.m_nSequence;


		g_Render->RenderText(money1.str(), ImVec2(aBox.right + 3, aBox.top + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
		iOffset += 12;
		g_Render->RenderText(money2.str(), ImVec2(aBox.right + 3, aBox.top + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
		iOffset += 12;
		g_Render->RenderText(money3.str(), ImVec2(aBox.right + 3, aBox.top + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
		iOffset += 12;
		g_Render->RenderText(money4.str(), ImVec2(aBox.right + 3, aBox.top + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
		iOffset += 12;
	}*/
	//if (pPlayer->EntIndex() != g_Globals.m_LocalPlayer->EntIndex())
	//{
	//	//g_Render->RenderRectFilled(10, 10, 300, 300, Color(30, 30, 30));
	//	//for (int i = 5; i < 13; i++)
	//	//{
	//		auto layer = pPlayer->m_AnimationLayers()[6];

	//		std::stringstream money1;
	//		money1 << _S("layer: ") << 6 << "  " << _S("m_flPlaybackRate:") << layer.m_flPlaybackRate;

	//		std::stringstream money2;
	//		money2 << _S("layer: ") << 6 << "  " << _S("m_flWeight:") << layer.m_flWeight;

	//		std::stringstream money3;
	//		money3 << _S("layer: ") << 6 << "  " << _S("m_flWeightDeltaRate:") << layer.m_flWeightDeltaRate;

	//		std::stringstream money4;
	//		money4 << _S("layer: ") << 6 << "  " << _S("m_flCycle:") << layer.m_flCycle;


	//		g_Render->RenderText(money1.str(), ImVec2(15, 15 + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
	//		iOffset += 12;
	//		g_Render->RenderText(money2.str(), ImVec2(15, 15 + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
	//		iOffset += 12;
	//		g_Render->RenderText(money3.str(), ImVec2(15, 15 + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
	//		iOffset += 12;
	//		g_Render->RenderText(money4.str(), ImVec2(15, 15 + iOffset), Color(218, 255, 82), false, true, g_Globals.m_Fonts.m_SegoeUI);
	//		iOffset += 12;
	//	//}
	//}
	...
	if (m_aPlayerData[pPlayer->EntIndex()].m_aSettings.m_Flags[3])
	{
		std::stringstream money;
		money << _S("$") << pPlayer->m_iAccount();

		g_Render->RenderText(money.str(), ImVec2(aBox.right + 3, aBox.top + iOffset), Color(104, 211, 95), false, true, g_Globals.m_Fonts.m_SegoeUI);
		iOffset += 12;
	}
	if (m_aPlayerData[pPlayer->EntIndex()].m_aSettings.m_Flags[4])
	{
		auto fakeducking = [&]() -> bool
		{
			...
			if (pPlayer->m_PlayerAnimStateCSGO()->m_flAnimDuckAmount) //-V550
			{
				if (pPlayer->m_PlayerAnimStateCSGO()->m_flAnimDuckAmount < 0.9f && pPlayer->m_PlayerAnimStateCSGO()->m_flAnimDuckAmount > 0.5f) //-V550
				{
					if (stored_tick != g_Globals.m_Interfaces.m_GlobalVars->m_iTickCount)
					{
						crouched_ticks[pPlayer->EntIndex()]++;
						stored_tick = g_Globals.m_Interfaces.m_GlobalVars->m_iTickCount;
					}

					return crouched_ticks[pPlayer->EntIndex()] > 16;
				}
				else
					crouched_ticks[pPlayer->EntIndex()] = 0;
			}

			return false;
		};

		if (fakeducking() && pPlayer->m_fFlags() & FL_ONGROUND && !pPlayer->m_PlayerAnimStateCSGO()->m_bLanding)
		{
			g_Render->RenderText(_S("FD"), ImVec2(aBox.right + 3, aBox.top + iOffset), Color(255, 0, 0, 255), false, true, g_Globals.m_Fonts.m_SegoeUI);
		}
	}
}
...
void C_PlayerESP::RenderGlow()
{
	for (auto i = 0; i < g_Globals.m_Interfaces.m_GlowObjManager->m_GlowObjectDefinitions.Count(); i++) {
		...
		glowObject.m_vGlowColor.x = color.r() / 255.0f;
		glowObject.m_vGlowColor.y = color.g() / 255.0f;
		glowObject.m_vGlowColor.z = color.b() / 255.0f;
