...
void C_CustomEventListener::FireGameEvent(C_GameEvent* pEvent)
{
	...
	else if (strstr(pEvent->GetName(), _S("player_hurt")))
	{
		...
		if (g_Globals.m_Interfaces.m_EngineClient->GetPlayerForUserID(pEvent->GetInt(_S("attacker"))) == g_Globals.m_Interfaces.m_EngineClient->GetLocalPlayer())
		{
			...
			if (g_Settings->m_aLogs[0])
			{
				...
				strHurtMessage += _S(" | resolver side: "); strHurtMessage += std::to_string(g_Globals.m_ResolverData.m_ResolverSide[pHurtPlayer->EntIndex()]);
				strHurtMessage += _S(" | delta: "); strHurtMessage += std::to_string(g_Globals.m_ResolverData.m_ResolverDelta[pHurtPlayer->EntIndex()]);
				strHurtMessage += _S(" | type: "); strHurtMessage += std::to_string(g_Globals.m_ResolverData.m_ResolverType[pHurtPlayer->EntIndex()]);

				g_LogManager->PushLog(strHurtMessage, _S("c"));
			}
		}
		else if (g_Settings->m_aLogs[1] && (g_Globals.m_Interfaces.m_EngineClient->GetPlayerForUserID(pEvent->GetInt(_S("attacker"))) != g_Globals.m_Interfaces.m_EngineClient->GetLocalPlayer()) && pHurtPlayer == g_Globals.m_LocalPlayer)
		{
			...
			g_LogManager->PushLog(strHurtMessage, _S("b"));
		}
	}
	...
	else if (strstr(pEvent->GetName(), _S("player_death")))
	{
		...
		g_Globals.m_RoundInfo.m_bShouldClearDeathNotices = true;
		g_Globals.m_LocalData.m_bDidBreakLBY = false;
		g_Globals.m_LocalData.m_flLowerBodyYaw = 0.0f;
		g_Globals.m_LocalData.m_flNextLowerBodyYawUpdate = 0.0f;
	}
	else if (strstr(pEvent->GetName(), _S("item_purchase")) && g_Settings->m_aLogs[2])
	{
		...
		g_LogManager->PushLog(strMessage, _S("a"));
	}
}